...
saveRDS(binary_ratings_filtered,
        file = here("input_data", "binary_preference_matrix.rds"))
...
algorithms <- list(
  ...
  # For IBCF: k = 10 or 30 did worse then k = 5
  "IBCF_10" = list(name = "IBCF", param = list(k = 5)),
  # For UBCF: nn = 100 and 500 did worse than nn = 50
  "UBCF_nn500" = list(name  = "UBCF", param = list(method = "Cosine", nn = 50))
)
...
# These are a specific users ones:
(random <- zoo_data %>%
  filter(user_id == 6) %>%
  pull(project_id) %>%
  as.character())
